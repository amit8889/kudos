...
const Analytics = ({ userData }) => {
  ...
  useEffect(() => {
    if (
      data?.kudosAllocationCount &&
      Array.isArray(data?.kudosAllocationCount) &&
      data.kudosAllocationCount.length > 0
    ) {
      const lables = [];
      const dataSets = [];
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white shadow rounded-lg p-6">
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody>
                  {data?.kudosLeaderboard &&
                    Array.isArray(data.kudosLeaderboard) &&
                    data.kudosLeaderboard.map((row, index) => (
          ...
          <div className="bg-white shadow rounded-lg p-6 mt-6">
            {data?.mostLikedKudos && (
              <p className="text-center">
                <strong>Most liked post:</strong>{" "}
                {`${data.mostLikedKudos?.sender?.name ?? "N/A"} gave ${
                  data.mostLikedKudos?.label ?? "N/A"
                } Badge to
          ${data.mostLikedKudos?.receiver?.name ?? "N/A"} — “${
                  data.mostLikedKudos?.reason ?? "N/A"
                }”`}
              </p>
            )}
          </div>
