...
const handleNavigate = ()=>{
  ...
  const handleSubmit = async(e) => {
    e.preventDefault();
    const customHeaders = { 'userid': userData._id };
    giveKudos('/api/kudos/addKudos','POST',kudosData,customHeaders,handleNavigate);
    //naigate to 
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg sticky top-0 z-10">
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Kudos List */}
        <div className="mb-4">
          <label htmlFor="receiver" className="block text-gray-700 font-semibold">Select users</label>
          <select
            id="receiver"
            name = "receiver"
          ...
          >
            <option value="" disabled>Select Kudos</option>
            {usersOptions && Array.isArray(usersOptions) && usersOptions.length>0 && usersOptions.map((kudos, index) => (
              <option  key={index} value={kudos._id}>{kudos.name}</option>
            ))}
          </select>
        </div>
        ...
        {/* Badge */}
        <div className="mb-4">
          <label htmlFor="badge" className="block text-gray-700 font-semibold">Select Badge</label>
          <select
            id="badge"
            name="badge"
          ...
          >
            ...
            {badgeOptions && Array.isArray(badgeOptions) && badgeOptions.length>0 && badgeOptions.map((badge, index) => (
              <option key={index} value={badge._id}>{badge.label}</option>
            ))}
          </select>
        </div>
